...
static void back_fill_data(FILE *fp, int width, int height, int fps)
{
    ...
    AVI_HDRL_LIST hdrl_list =
    {
        ...
        {'h', 'd', 'r', 'l'},
        {
            ...
            1000000 / fps, 25000, 0, 0, nframes, 0, 1, JPEG_MAX_SIZE, width, height,
            {0, 0, 0, 0}
        },
        {
            {'L', 'I', 'S', 'T'},
            ...
            {'s', 't', 'r', 'l'},
            {
                ...
                {'v', 'i', 'd', 's'},
                {'J', 'P', 'E', 'G'},
                0, 0, 0, 0, 1, fps, 0, nframes, JPEG_MAX_SIZE, 0xFFFFFF, 0,//第五六个可以控制帧率，fps=rate/scale
                {0, 0, width, height}
            },
            {
                {'s', 't', 'r', 'f'},
...
void jpeg2avi_add_frame(FILE *fp, void *data, unsigned int len)
{
    ...
    fwrite(tmp, 4, 1, fp);          // 写入帧标记
    fwrite(&len, 4, 1, fp);     // 写入对齐后的长度
    fwrite(data, len, 1, fp);       // 写入 JPEG 实际数据
