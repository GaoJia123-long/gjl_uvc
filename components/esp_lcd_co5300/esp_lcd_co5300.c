...
esp_err_t esp_lcd_new_panel_co5300(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    case 16: // RGB565
        ...
        co5300->fb_bits_per_pixel = 16;
        break;
    case 18: // RGB666
        co5300->colmod_val = 0x66;
        // each color component (R/G/B) should occupy the 6 high bits of a byte, which means 3 full bytes are required for a pixel
        co5300->fb_bits_per_pixel = 24;
        break;
    default:
...
static const co5300_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // {0x35, (uint8_t []){0x00}, 1, 0},
    // {0xFE, (uint8_t []){0x00}, 1, 0},
    // {0xC4, (uint8_t []){0x80}, 1, 0},// SPI 
    // // {0x36, (uint8_t []){0x00}, 1, 0},
    // //{0x3A, (uint8_t []){0x55}, 1, 0},//
    // {0x53, (uint8_t []){0x20}, 1, 0},
    // {0x63, (uint8_t []){0xFF}, 1, 0},
    // {0x2A, (uint8_t []){0x00, 0x00, 0x01, 0xF5}, 4, 0},
    // {0x2B, (uint8_t []){0x00, 0x00, 0x01, 0x99}, 4, 0},
    // {0x29, (uint8_t []){0x00}, 0, 60},  //display on
    // {0x51, (uint8_t []){0x98}, 1, 0},   //Brightness
...
static esp_err_t panel_co5300_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        if ((init_cmds[i].cmd ==  0xF0)) {
            is_user_set = ((uint8_t *)init_cmds[i].data)[0] == 0x00 ? true : false;
        }
    }
...
esp_err_t panel_co5300_set_brightness(esp_lcd_panel_t *panel, uint8_t brightness)
{
    co5300_panel_t *co5300 = __containerof(panel, co5300_panel_t, base);
    esp_lcd_panel_io_handle_t io = co5300->io;
    ESP_RETURN_ON_ERROR(tx_param(co5300, io, LCD_CMD_WRDISBV, (uint8_t[]) {
        brightness
    }, 1), TAG, "send command failed");
    return ESP_OK;
}
